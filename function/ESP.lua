...
function InitMain()
	...
	RemoveToBlacklist.Text = "从黑名单中移除"
	...
	RemoveToWhitelist.Text = "从白名单中移除"
...
提示: 名称标签/人物上色 颜色可能出现问题.
提示 #2: The blacklist feature will return! I just didn't have enough time to make the gui.
提示 #3: Save Settings might still be bugged. Message me if it's fucked up still.

这适用于每场比赛，尽管 Aimbot 不行！（不适用于：越狱和Phantom Forces）

FAQ:
Q:锁头怎么用?
A: 激活它，然后在游戏中按住右键单击。瞄准机器人将锁定最近的敌人，而不是在墙后。（如果所述玩家在墙后，它将找到下一个不在墙后的最近玩家。

Q:我是如何检测到玩家何时在墙后?
A: 将摄像机光线投射到其他玩家r.

Q: 使用 aimbot 时我的子弹仍然不中?!
A: 责怪子弹传播，尝试控制你开火的频率。
]]
	...
	TracersUnderChars.Name = "人物连线在字符下"
	...
	SaveBlacklist.MouseButton1Click:connect(function()
		...
		SaveBlacklist.Text = "保存完成!"
		wait(1)
		SaveBlacklist.Text = "保存黑名单"
	end)
	...
	SaveSettings.MouseButton1Click:connect(function()
		SaveBullshitSettings()
		SaveSettings.Text = "保存完成!"
		wait(1)
		SaveSettings.Text = "保存"
	end)
